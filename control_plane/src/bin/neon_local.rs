...
use neon_broker::DEFAULT_LISTEN_ADDR as DEFAULT_BROKER_ADDR;
...
[broker]
broker_endpoints = '{DEFAULT_BROKER_ADDR}'
...
fn handle_start_all(sub_match: &ArgMatches, env: &local_env::LocalEnv) -> anyhow::Result<()> {
    ...
    if let Err(e) = pageserver.start(&pageserver_config_overrides(sub_match)) {
        eprintln!("pageserver start failed: {e}");
        try_stop_neon_broker_process(env);
        exit(1);
    }
...
fn try_stop_neon_broker_process(env: &local_env::LocalEnv) {
