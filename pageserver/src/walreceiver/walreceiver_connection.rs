...
pub async fn handle_walreceiver_connection(
    ...
    info!("connecting to {wal_source:?}");
    assert_eq!(wal_source.get_application_name(), None);
    assert_eq!(wal_source.get_replication_mode(), None);
    let wal_source = {
        let mut w = wal_source.clone();
        w.application_name("pageserver")
            .replication_mode(tokio_postgres::config::ReplicationMode::Physical);
        w
    };

    let (mut replication_client, connection) =
        time::timeout(connect_timeout, wal_source.connect(postgres::NoTls))
            .await
