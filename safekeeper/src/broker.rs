...
use neon_broker::neon_broker_proto::Empty;
use neon_broker::neon_broker_proto::SubscribeSafekeeperInfoRequest;
use neon_broker::parse_proto_ttid;
use tonic::Request;
...
async fn push_loop(conf: SafeKeeperConf) -> anyhow::Result<()> {
    let mut client = NeonBrokerClient::connect(conf.broker_endpoints.clone()).await?;
...
async fn pull_loop(conf: SafeKeeperConf) -> Result<()> {
    ...
    let request = SubscribeSafekeeperInfoRequest {
        subscription_key: Some(ProtoSubscriptionKey::All(Empty {})),
    };
    ...
    while let Some(msg) = stream.message().await? {
        let proto_ttid = msg
            .tenant_timeline_id
            .as_ref()
            .ok_or(anyhow!("missing tenant_timeline_id"))?;
